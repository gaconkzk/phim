...
exports.findMedias = (url) => {
  return findEpisodeUrl(url)
    .then(u => Promise.all([Promise.resolve({url, token: u.token}), urllib.request(u.epiUrl)]))
    .then(result => ({ slug: result[0].url, token: result[0].token, data: result[1].data}))
    .then(html => ({slug: html.slug, token: html.token, lines: html.data.toString("utf-8").split("\n") }))
    .then(takeMediaInfo)
    ...
    .then(media => {
      ...
      media.medias = media.medias.map(video => ({
...
const takeMediaInfo = ({slug, token, lines}) => {
  ...
  console.log(' test data with key')
  console.log(decodeUrl(json.medias[0].url, 'PhimMoi.Net@' + json.episodeId, json.decryptkey))
  console.log(decodeUrl(json.embedUrls[0], 'PhimMoi.Net@' + json.episodeId, json.decryptkey))
