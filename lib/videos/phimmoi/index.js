...
const defaultHeaders = {
  ...
  // test proxy
  // enableProxy: true,
  // proxy: 'http://proxy.tma.com.vn:8080'
}
...
function _search(url) {
  return urllib.request(url, defaultHeaders)
    .then((response) => {
...
const findLinks = (json) => {
  ...
  if (haveLink) {
    ...
    return urllib.request(thirdpt, Object.assign({}, defaultHeaders, {
      headers: {
...
const findEpisodeUrl = (id) => {
  ...
  return urllib
    .request(referer, defaultHeaders)
...
exports.watchmovie = ({url, referer}) => {
  return urllib.request(url, Object.assign({}, defaultHeaders, {
    headers: {
      referer
    }
  })).then(res => {
...
exports.segment = ({url, referer}, segment, writeStream ) => {
  ...
  return urllib.request(surl, Object.assign({}, defaultHeaders, {
    headers: {
      referer
    },
    writeStream
